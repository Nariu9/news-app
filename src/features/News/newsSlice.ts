...
export const fetchArticles = createAsyncThunk<ArticleType[], undefined, { rejectValue: string }>('news/fetchArticles', async (arg, {
    rejectWithValue,
    dispatch
}) => {
...
export const fetchArticleById = createAsyncThunk<ArticleType, string, { rejectValue: string }>('news/fetchArticleById', async (articleId, {
    rejectWithValue,
    dispatch
}) => {
    dispatch(changeRequestStatus({loading: true}))
